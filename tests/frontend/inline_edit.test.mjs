...
test("inline quick-edit PUTs exact minutes + notes without opening the form", async () => {
  ...
  await page.route("**/api/timeactivity/*", (route) => {
    ...
    const body = r.postDataJSON(); return route.fulfill({ json: { Id: "E1", SyncToken: "8", operationId: body.operation_id } });
  });
  ...
  await page.fill("#entries .entry.ie .ie-dur", "3.2"); // exact 3 hours, 12 minutes
  ...
  assert.equal(puts[0].minutes, 12);
  assert.equal(puts[0].sync_token, "7");
  assert.match(puts[0].operation_id, /^[0-9a-f-]{36}$/);
